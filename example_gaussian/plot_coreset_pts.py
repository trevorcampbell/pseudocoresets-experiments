...
trial_num = 10
np.random.seed(42)
nms = [('BPSVI', 'PSVI', np.load('results/results_BPSVI_1.pk', allow_pickle=True)), ('SVI', 'SparseVI', np.load('results/results_SVI_1.pk', allow_pickle=True))]
  ...
  if d>2: # fix this to make it independent of ordering of input
    ...
    if size_x_axis:
      ...
      if m==0:
        ...
        plot_gaussian(fig, muw_[m,:], (4./9.)*Sigw_[m,:], (4./9.)*Sig_, 'maroon', 17, 9, 1, alphagauss, 'solid', nms[0][1])
    if size_x_axis:
      fig.scatter(svipm_[:, 0], svipm_[:, 1], fill_color='blue', alpha=0.6, size=sizebase*(sviw_[m]>0)+4*sizebase*sviw_[m]/sviw_[m].max(), line_color=None)
    else:
      fig.scatter(svipm_[:, 0], svipm_[:, 1], fill_color='blue', alpha=0.6, size=sizebase*(sviw_[svimsz, :]>0)+4*sizebase*sviw_[m,:]/sviw_[m,:].max(), line_color=None)
    if size_x_axis:
